<template>
	<div :style='{"width":"100%","padding":"20px","margin":"10px auto","position":"relative","background":"none","maxWidth":"1200px"}'>
		...
		<div class="section-title" :style='{"padding":"0","margin":"0 0 30px 0","color":"#fff","textAlign":"center","background":"url(http://codegen.caihongy.cn/20250206/913c65e0ee3248549258d770e7c33614.png) no-repeat center","width":"100%","letterSpacing":"2px","lineHeight":"80px","fontSize":"22px","fontWeight":"600"}'>我的订单</div>
		...
		<el-pagination
			...
			:total="total"
			:style='{"padding":"0 calc(50% - 600px)","margin":"10px auto","whiteSpace":"nowrap","color":"#333","width":"100%","fontWeight":"500","order":"6"}'
							...
							if(item.type==1||item.type==3) {
								...
								this.$http.post(table+'/update', this.user).then(res => {
									...
									this.$http.post(`orders/update`, item).then(res => {
										if(item.tablename == `chongwushangcheng`){
											// 加回库存数量
											good.alllimittimes = good.alllimittimes + item.buynumber;
											this.$http.post(`chongwushangcheng/update`, good).then(res => {
											});
										}
										this.$message({
