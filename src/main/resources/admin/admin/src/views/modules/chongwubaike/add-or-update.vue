	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="宠物分类" prop="chongwufenlei" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="宠物习性" prop="chongwuxixing" >
					<el-input v-model="ruleForm.chongwuxixing" placeholder="宠物习性" clearable  :readonly="ro.chongwuxixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="宠物习性" prop="chongwuxixing" >
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboota7e2182m/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chongwubaike/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chongwubaikeCrossAddOrUpdateFlag = false;
											this.parent.search();
