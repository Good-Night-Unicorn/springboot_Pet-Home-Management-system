...
<script>
	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					url: `discusschongwushangcheng/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `discusschongwushangcheng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusschongwushangchengCrossAddOrUpdateFlag = false;
											this.parent.search();
